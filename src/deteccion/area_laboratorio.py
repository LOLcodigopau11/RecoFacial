...
AREA_OBJETIVO = "Laboratorio de Desarrollo"  # Área a simular
...
def obtener_info_area(nombre_area):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id_area, nivel_seguridad FROM areas WHERE nombre_area = %s",
        (nombre_area,),
    )
    resultado = cursor.fetchone()
    conn.close()
...
def mostrar_texto(frame, texto, posicion, color=(0, 255, 0)):
    ...
    cv2.putText(frame, texto, posicion, cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
...
def procesar_acceso():
    ...
    try:
        while True:
            ...
            empleado_info = (
                obtener_info_empleado(dni_predicho) if confianza < 80 else None
            )
            ...
            if empleado_info:
                ...
                if nivel_acceso >= nivel_seguridad_area:
                    ...
                    registrar_acceso(
                        id_empleado, id_area, True, "reconocimiento_facial"
                    )
                else:
                    color = (0, 0, 255)
                    mensaje = "Acceso denegado"
                    registrar_acceso(
                        id_empleado, id_area, False, "reconocimiento_facial"
                    )
            ...
            if key == ord("q"):
                ...
                "n"
            ):  # Espera a que se presione 'n' para continuar con un nuevo intento
